...
// rotation is CSS (see .media-spinner in _layout.scss), not SMIL.
const SPINNER_SVG =
...
export default function remarkFigures() {
  return (tree) => {
    tree.children = tree.children.map((node) => {
      ...
      // the same .media-spinner overlay the project tiles show, faded out by
      ...
      const markLoaded = `this.closest('figure').classList.add('is-loaded')`;
      ...
      return {
        ...
        value:
          ...
          `<span class="media-spinner">${SPINNER_SVG}</span>` +
